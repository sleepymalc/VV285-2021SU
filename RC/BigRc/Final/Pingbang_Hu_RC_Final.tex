...
\begin{frame}
    ...
    \visible<6->{
        \begin{equation*}
            \int_\Omega(\nabla u)^2\,dx = 0
        \end{equation*}
        which implies $\nabla u(x) = 0$ for all $x$, i.e.}\visible<7->{, u is constant.}\visible<8->{ Since $\at{u}{\partial \Omega}{} = 0$}\visible<9->{, this would mean $u(x) = 0$ for all $x$, which we have excluded.}\visible<10->{ Therefore, $\lambda = 0$ is impossible and we conclude that
        \begin{equation*}
            \lambda < 0.
        \end{equation*}
